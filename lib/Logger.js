...
class Logger {
    ...
    constructor(log_level, config){
        ...
        if(config.logstash_transport != null){
            config.logstash_transport.timestamp = dateFunc;
            this.logstashTransport = new (winston.transports.Logstash)(config.logstash_transport);
        ...
        if(config.file_name != null){
            this.fileTransport = new (winston.transports.File)({ filename: config.file_name, timestamp:dateFunc, json: false});
    ...
    handleSIGHUP(){
        var fullname = this.config.file_name;
